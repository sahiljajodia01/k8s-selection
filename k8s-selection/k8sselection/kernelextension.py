...
class K8sSelection:
    ...
    def open_kubeconfig(self):
        try:
            with io.open(os.environ['HOME'] + '/.kube/config', 'r', encoding='utf8') as stream:
                load = yaml.safe_load(stream)
            return load
        except:
            raise Exception("Cannot load kubeconfig file!")

    def update_kubeconfig(self, load):
        try:
            ...
            raise Exception("Cannot write to kubeconfig file!")

    def get_namespace_from_context(self, load, context):
        namespace = 'default'
    ...
    def get_clustername_from_context(self, load, context):
        for i in load['contexts']:
            if i['name'] == context:
    ...
    def get_serverip_from_cluster(self, load, cluster):
    ...
    def get_user_from_context(self, load, context):
    ...
    def get_context_object_from_context_name(self, contexts, context_name):
        for i in range(len(contexts)):
            if contexts[i]['name'] == context_name:
                context = contexts[i]
                break
    ...
    def handle_comm_message(self, msg):
        ...
        if action == 'Refresh':
            ...
            self.change_current_context(msg)
            ...
            # This action get the kerberos ticket and sends the message to the frontend
            self.kerberos_auth(msg)
        elif action == 'check-auth-required':
            # This method checks whether kerberos ticket is already there or not
            self.check_auth_required(msg)

    def delete_current_context(self, msg):
        self.log.info("Deleting context from KUBECONFIG")
        context = msg['content']['data']['context']

        try:
            # Open the file
            load = self.open_kubeconfig()

            # Get the user to delete
            user = self.get_user_from_context(load, context)

            # Delete the given cluster, context and user
            for i in range(len(load['contexts'])):
                if load['contexts'][i]['name'] == context:
                    load['contexts'].pop(i)
                    break
    ...
    def check_auth_required(self, msg):
        context = msg['content']['data']['context']

        try:
            load = self.open_kubeconfig()

            namespace = self.get_namespace_from_context(load, context)
    ...
    def create_user(self, msg):
        ...
        try:
            ...
            selected_cluster = self.get_clustername_from_context(load, selected_context)
            ...
            # If helm is initialized successfully then deploy the helm chart
            if out.decode('utf-8') != '':
    ...
    def add_local_context(self, msg):
        ...
        # The main logic of adding cluster and contexts to kubeconfig file
        try:
    ...
    def add_openstack_context(self, msg):
    ...
    def change_current_context(self, msg):
        context = msg['content']['data']['context']
        tab = msg['content']['data']['tab']
        ...
        try:
            ...
            # Opening the KUBECONFIG YAML file using the yaml library
            load = self.open_kubeconfig()

            # Getting the namespace of the context
            namespace = self.get_namespace_from_context(load, context)
            ...
            # Extracting the cluster and server IP of the currently selected context
            cluster_name = self.get_clustername_from_context(load, load['current-context'])
            server_ip = self.get_serverip_from_cluster(load, cluster_name)
